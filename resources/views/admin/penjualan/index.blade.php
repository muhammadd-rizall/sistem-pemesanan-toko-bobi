        ...
        <div class="max-w-7xl mx-auto">
                ...
                <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
                    <div class="flex items-start justify-between">
                        <div>
                            ...
                            <h3 class="text-2xl font-bold text-gray-800">Rp
                                {{ number_format($rataRataTransaksi, 0, ',', '.') }}</h3>
                        </div>
            ...
            <div class="bg-white rounded-xl shadow-sm p-6 mb-6 border border-gray-100">
                <div class="flex flex-wrap items-center justify-between gap-4">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <form method="GET" action="{{ route('penjualan.index') }}" class="flex items-center gap-3">
                            <select name="filter" onchange="this.form.submit()"
                                class="px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 bg-gray-50">
                                <option value="semua" {{ $filter == 'semua' ? 'selected' : '' }}>Semua</option>
                                <option value="hari_ini" {{ $filter == 'hari_ini' ? 'selected' : '' }}>Hari Ini</option>
                                <option value="minggu_ini" {{ $filter == 'minggu_ini' ? 'selected' : '' }}>Minggu Ini
                                </option>
                    ...
                    <div class="flex gap-3">
                        ...
                        <a href="{{ route('penjualan.export', ['filter' => $filter]) }}"
                            class="px-6 py-2.5 bg-green-100 hover:bg-green-200 text-green-700 rounded-lg font-medium transition-colors flex items-center gap-2">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="p-6 border-b border-gray-100">
                    <h2 class="text-lg font-semibold text-gray-800">Data Penjualan -
                        {{ ucfirst(str_replace('_', ' ', $filter)) }}</h2>
                </div>
                                ...
                                @foreach ($penjualan as $item)
                                    <tr class="hover:bg-gray-50 transition-colors">
                                        <td class="px-6 py-4 text-sm font-medium text-gray-800">
                                            {{ $item->kode_penjualan }}</td>
                                        <td class="px-6 py-4 text-sm text-gray-600">{{ $item->tanggal->format('d/m/Y') }}
                                        </td>
            ...
            <form id="modalForm" method="POST" action="{{ route('penjualan.store') }}">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="number" id="hargaInput" name="harga_satuan" min="0" required
                            class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Total</label>
                        <input type="number" id="totalInput" name="total" readonly
                    ...
                    <div>
                        ...
                        <textarea name="keterangan" rows="3"
                            class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"></textarea>
                    </div>
                </div>
