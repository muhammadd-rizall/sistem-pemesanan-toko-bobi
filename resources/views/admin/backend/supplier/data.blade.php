...
-center">
                ...
                <button type="button" class="text-green-700 hover:text-green-900"
                    ...
                    <svg class="w-5 h-5" fill="currentColor" viewBox d="M6 18L18 6M6 6l12 12">
                        </path>
                    </svg>
                </button>
            </div>
                    ...
                    <table class="table-fixed w-full divide-y divide-sage-100">
                        <thead class="bg-[#7eb17e] text-black uppercase text-xs tracking-wider">
                            <tr>
                                ...
                                <th class="px-2 py-4 text-center font-bold w-[25%]">Nama Perusahaan</th>
                                <th class="px-6 py-4 text-center font-bold w-[25%]">Kontak Person</th>
                                <th class="px-6 py-4 text-center font-bold w-[20%]">No Telephone</th>
                                <th class="px-6 py-4 text-center font-bold w-[25%]">Email</th>
                                <th class="px-6 py-4 text-center font-bold w-[20%]">Provinsi</th>
                                <th class="px-6 py-4 text-center font-bold w-[20%]">Kota</th>
                                <th class="px-6 py-4 text-center font-bold w-[20%]">Kecamatan</th>
                                <th class="px-6 py-4 text-center font-bold w-[30%]">Alamat</th>
                                <th class="px-6 py-4 text-center font-bold w-[15%]">Status</th>
                            ...
                            @forelse ($datas as $index => $data)
                                <tr class="hover:bg-sage-50/50 transition-colors duration-200 animate-fade-in-up"
                                    ...
                                    <td class="px-6 py-4 text-center font-semibold align-top">
                                        <p class="text-sm text-gray-600">{{ $data->nama_perusahaan }}</p>
                                    </td>

                                    <!-- Kontak person -->
                                    <td class="px-6 py-4 text-center font-semibold align-top">
                                        <p class="text-sm text-gray-600">{{ $data->kontak_person }}</p>
                                    </td>

                                    <!-- No Telephone -->
                                    <td class="px-6 py-4 text-center font-semibold align-top">
                                        <p class="text-sm text-gray-600">{{ $data->no_telephone }}</p>
                                    </td>

                                    <!-- Email -->
                                    <td class="px-6 py-4 text-center font-semibold align-top"></td>
                                        <p class="text-sm text-gray-600">{{ $data->email }}</p>
                                    </td>

                                    <!-- Provinsi -->
                                    <td class="px-6 py-4 text-center font-semibold align-top">
                                        <p class="text-sm text-gray-600">{{ $data->provinsi }}</p>
                                    </td>

                                    <!-- Kota -->
                                    <td class="px-6 py-4 text-center font-semibold align-top">
                                        <p class="text-sm text-gray-600">{{ $data->kota }}</p>
                                    </td>

                                    <!-- Kecamatan -->
                                    <td class="px-6 py-4 text-center font-semibold align-top">
                                        <p class="text-sm text-gray-600">{{ $data->kecamatan }}</p>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 text-center align-top text-sm font-medium">
                                        <div class="flex items-center justify-center gap-2">
                                            ...
                                            <form action="{{ route('deleteSupplier', $data->id) }}" method="POST"
                                                class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
    ...
    <script>
        setTimeout(function() {
            ...
            if (alert) {
                alert.classList.add('opacity-0', 'transition', 'duration-500'); // fade out
    ...
    {{-- CSS untuk Animasi (Bisa dipindah ke file CSS utama jika diinginkan) --}}
    <style>
        @keyframes fade-in {
            from {
                opacity: 0;
