...
@section('content')
    <div class="max-w-4xl mx-auto px-4 py-6">
        <div class="bg-white rounded-3xl shadow-2xl p-8">
            <h3 class="text-3xl font-bold mb-8 text-gray-800 text-center">Edit Data Diskon</h3>

            <form action="{{ route('updateDiskon', $data->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="mb-4">
                    ...
                    focus:ring focus:outline-none rounded-md py-2 px-2 {{ $errors->has('kode_diskon') ? 'border-red-500' : '' }}"
                        value="{{ old('kode_diskon', $data->kode_diskon) }}" required>
                    @error('kode_diskon')
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="nilai_diskon" id="nilai_diskon"
                        class="mt-2 block w-full border border-gray-500 text-black focus:border-blue-300 focus:ring-blue-200
                    focus:ring focus:outline-none rounded-md py-2 px-2 {{ $errors->has('nilai_diskon') ? 'border-red-500' : '' }}"
                        value="{{ old('nilai_diskon', $data->nilai_diskon) }}" required>
                    @error('nilai_diskon')
                        <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="date" name="tanggal_mulai" id="tanggal_mulai"
                        class="mt-2 block w-full border border-gray-500 text-black focus:border-blue-300 focus:ring-blue-200
                    focus:ring focus:outline-none rounded-md py-2 px-2 {{ $errors->has('tanggal_mulai') ? 'border-red-500' : '' }}"
                ...
                <div class="mb-4">
                    ...
                    <input type="date" name="tanggal_berakhir" id="tanggal_berakhir"
                        class="mt-2 block w-full border border-gray-500 text-black focus:border-blue-300 focus:ring-blue-200
                    focus:ring focus:outline-none rounded-md py-2 px-2 {{ $errors->has('tanggal_berakhir') ? 'border-red-500' : '' }}"
                        value="{{ old('tanggal_berakhir', $data->tanggal_berakhir) }}" required>
