    ...
    <div class="max-w-7xl mx-auto px-4 py-6">
        ...
        <div class="animate-fade-in">
            ...
            <div class="flex flex-col sm:flex-row items-center justify-between gap-4 mb-8">
                <div>
                    <h1 class="text-3xl font-bold text-sage-800">Manajemen Review Produk</h1>
                    <p class="text-sage-600 mt-1">Kelola semua data Review di toko Anda.</p>
                </div>
                <div class="flex items-center gap-4 w-full sm:w-auto">
                    ...
                    {{-- <a href="#"
                        ...
                        <span class="whitespace-nowrap">Tambah Review</span>
                    </a> --}}
                </div>
            </div>
                    ...
                    <table class="min-w-max w-full text-sm sm:text-base divide-y divide-sage-100">
                        <thead class="bg-[#7eb17e] text-black uppercase text-xs tracking-wider">
                            <tr>
                                ...
                                <th class="px-2 py-2 sm:px-4 sm:py-3 text-center font-bold">Nama Produk</th>
                                <th class="px-2 py-2 sm:px-4 sm:py-3 text-center font-bold">Nama Customer</th>
                                <th class="px-2 py-2 sm:px-4 sm:py-3 text-center font-bold hidden md:table-cell">Rating</th>
                                <th class="px-2 py-2 sm:px-4 sm:py-3 text-center font-bold hidden md:table-cell">Comment</th>
                        ...
                        <tbody class="divide-y divide-sage-100">
                            @forelse ($datas as $index => $data)
                                <tr class="hover:bg-sage-50 transition duration-200">
                                    ...
                                    <td class="px-2 py-2 sm:px-4 sm:py-3 text-center">{{ $data->produk->nama_produk }}</td>
                                    <td class="px-2 py-2 sm:px-4 sm:py-3 text-center">{{ $data->customer->name }}</td>
                                    <td class="px-2 py-2 sm:px-4 sm:py-3 text-center hidden md:table-cell">{{ $data->rating }}</td>
                                    <td class="px-2 py-2 sm:px-4 sm:py-3 text-center hidden md:table-cell">{{ $data->comment }}</td>
                                    <td class="px-2 py-2 sm:px-4 sm:py-3 text-center">
                                        <div class="flex items-center justify-center gap-2">
                                            ...
                                            <form action="{{ route('deleteReview', $data->id) }}" method="POST"
                                                class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
